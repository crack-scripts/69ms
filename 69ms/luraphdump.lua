print(25,"starting the dummppP!!! Made by 69ms, join discord.gg/69ms",25)
...
local methods={
    ...
    },
    {
        ...
        --     local balls=[[local _realvar=%s;%s=setmetatable({},{__index=function(o,J)local V=_realvar[J]_69ms(V)return V end})]]
...
local _69msrobloxenv=(not commercial and require("./fakegame")) or {}
local _tbl=_69msrobloxenv._tbl
local _pcall=pcall
...
local genv={}
local cenv = {}
for i,v in _69msrobloxenv do
    cenv[i] = v
end
for i,v in roblox do
    cenv[i] = v
end
local _game=(not commercial and (function()
    local ok,place=pcall(function() return roblox.deserializePlace(fs.readFile("Baseplate.rbxl")) end)
    return ok and place or nil
end)()) or {}
...
cenv.loadstring=function(src,b)
    ...
    if src=="_LOL_Replace_69ms_" then
...
local fakeenv -- the environment the chunk runs in (assigned just before the pcall below)
cenv.getfenv=function(lvl)
        -- Always hand back the fake environment so luraph scripts that do
        -- getfenv().script / getfenv()[n] don't hit nil. The old code returned
        -- the never-defined `fenv_mt`, which caused most of the
        -- "attempt to index nil with ..." dump errors.
...
local found={}
-- `fakeenv` is the forward-declared local from the getfenv() fix above. Unknown
-- globals fall back to the forgiving _tbl instead of nil, so the luraph VM
-- doesn't crash on `nil.something` / `nil[n]`.
fakeenv=setmetatable({},{__index=function(_,key)
...
local result=sanitize_paths(table.concat(r,"\n"))
fs.writeFile(outpath..targetfilename:gsub(".lua","")..post,"--// This file was generated by 69ms V4 [https://discord.gg/gEHQbMP872]\n"..result)

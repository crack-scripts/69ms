...
/**
 * .l command - env-log using httplog2.lua
...
async function run(message, flags) {
  ...
  loadingCont.addTextDisplayComponents(body("Running env-log…"));
  ...
  const animationInterval = setInterval(async () => {
    ...
    spinCont.addTextDisplayComponents(body(`Running env-log… ${spinner}`));
  ...
  try {
    ...
    container.addTextDisplayComponents(header("✅", "Env-Log Complete"));
...
module.exports = {
  trigger: ".l",
  description: "Env-log Lua scripts using httplog2.lua",
  run,
};

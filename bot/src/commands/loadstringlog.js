...
/**
 * .loadstringlog command - capture loadstring payloads
...
async function run(message, flags) {
  ...
  loadingCont.addTextDisplayComponents(body("Capturing loadstring payloads…"));
  ...
  const animationInterval = setInterval(async () => {
    ...
    spinCont.addTextDisplayComponents(body(`Capturing loadstring payloads… ${spinner}`));
  ...
  try {
    ...
    container.addTextDisplayComponents(header("✅", "Loadstring Log Complete"));
    container.addSeparatorComponents(new SeparatorBuilder().setDivider(true).setSpacing(4));

    const bodyText =
      (files.length > 0
        ? `**Captured:** ${files.length} log(s)\n**Total time:** ${totalTime.toFixed(2)}s`
    ...
    clearInterval(animationInterval);
    console.error("[loadstringlog] error:", e);
...
module.exports = {
  trigger: ".loadstringlog",
  description: "Capture loadstring payloads",
  run,
};
